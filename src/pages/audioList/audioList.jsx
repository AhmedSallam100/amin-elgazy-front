...
function PlayerIcons({ id }) {
  ...
  const handleShareClick = async () => {
    const videoUrl = `${clientUrl}/audio-list/${id}`;
    try {
      ...
      >
        {audios.length === 0 && (
          <div className="no-exam-message" style={{ marginTop: 0 }}>
            ...
            <h1>لا يوجد تسجيلات</h1>
            <Button text="الصفحة الرئيسية" path={`/`} />
          </div>
        )}
        {audios.length > 0 && <h1>قائمه التسجيلات</h1>}
        <ul>
          {audios.map((audio) => (
            <li key={audio._id}>
              <span>{audio.title}</span>
              <PlayerIcons id={audio._id} videoUrl={audio.video} />
            </li>
          ))}
        </ul>
      </div>
